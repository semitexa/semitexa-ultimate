...
- **Nothing else is canonical.** Not `var/docs/`, not `var/docs/FOR_IMPLEMENTATION/`, not `vendor/semitexa/docs/` (or `packages/semitexa-docs/` in a workspace/monorepo layout), not chat history, not `AI_NOTES.md`, not design documents, not commit messages.
